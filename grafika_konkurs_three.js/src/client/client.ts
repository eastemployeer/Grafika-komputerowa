...
const planeGeometry: THREE.PlaneGeometry = new THREE.PlaneGeometry(100, 100)
...
fbxLoader.load(
    ...
    (fbx) => {
        ...
        let animationAction = mixer.clipAction((fbx as any).animations[0])
        ...
        modelMesh = fbx;
                ...
                fbxLoader.load('models/aj@stop.fbx',
                    (fbx) => {
                        ...
                        fbxLoader.load('models/aj@running.fbx',
                            (fbx) => {
                                console.log("loaded running");
                                (fbx as any).animations[0].tracks.shift() //delete the specific track that moves the object forward while running
                                let animationAction = mixer.clipAction((fbx as any).animations[0]);
                                animationActions.push(animationAction)
...
for (let i = 0; i < 100; i++) {
    ...
    const material: THREE.MeshPhongMaterial = new THREE.MeshPhongMaterial({shininess: 50, reflectivity: 1, refractionRatio: 0.98, color: 0xffffff, emissive: 0x0, specular: 0x898989})
    switch (i%3) {
                case 0:
...
spheres.forEach((c) => {
    ...
    c.position.y = ((c.geometry.boundingBox as THREE.Box3).max.y - (c.geometry.boundingBox as THREE.Box3).min.y) / 2 + Math.random()*4;
    scene.add(c)
});
