        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                ...
                if(j==1 && i==1){
                    ...
                    fillMiddleSquare(new_down_left,new_down_right,new_up_right,new_up_left); //TO WYWOLANIE TUTAJ WYPELNIA TYLKO NAJMNIEJSZY KWADRAT SRODKOWY ZE WSZYSTKICH ITERACJI
                }
                else {
                    glBegin(GL_POLYGON);
...
void render(){
    ...
    glFlush(); //przekazanie polecen rysujacych do wykonania
}
